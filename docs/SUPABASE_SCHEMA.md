...
Phase 1 gives ADIA a secure database foundation for deployment visibility. Phase 2B extends it with raw evidence-file metadata for fixture ingestion. The schema stores organizations, projects, deployment runs, raw evidence metadata, Terraform evidence, IaC scan findings, deterministic anomaly records, insight records, recommendations, and evidence links.

These phases do not wire Supabase into the browser application. They also do not add webhook ingestion APIs, parser execution, deterministic analyzer execution, LLM generation, live dashboard queries, Edge Functions, or Terraform apply behavior.
...
`raw_evidence_files` stores fixture evidence metadata only:

- Evidence kind and format.
- Safe relative fixture path.
- Optional label.
- File size and SHA-256 hash.
- JSON metadata copied from the validated fixture envelope.

It does not store raw file contents, parse Terraform, parse Checkov, call LLMs, or execute infrastructure commands.
...
Phase 2B fixture ingestion writes new deployment runs and raw evidence metadata at runtime; the seed file remains deterministic static demo data.
