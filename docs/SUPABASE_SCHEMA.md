...
- Anomaly engine versions must be present, anomaly fingerprints must be valid SHA-256 values when present, anomaly evidence refs cannot contain nulls, and anomaly metadata must be a JSON object.
...
Phase 2B fixture ingestion and Phase 2E verified webhook persistence write new deployment runs and raw evidence metadata at runtime; the seed file remains deterministic static demo data. Phase 3D and Phase 4C do not change seed data.
