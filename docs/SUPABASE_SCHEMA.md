...
Phase 1 gives ADIA a secure database foundation for deployment visibility. Phase 2B extends it with raw evidence-file metadata for fixture ingestion, and Phase 2E reuses that path for verified GitHub webhook persistence. The schema stores organizations, projects, deployment runs, raw evidence metadata, Terraform evidence, IaC scan findings, deterministic anomaly records, insight records, recommendations, and evidence links.

These phases do not wire Supabase into the browser application. They also do not add parser execution, deterministic analyzer execution, parser output persistence, LLM generation, live dashboard queries, Edge Functions, artifact download, or Terraform apply behavior.
...
Phase 3C documents how future parser output should be persisted, but does not add a migration or write code.
...
Before implementation, the schema should gain replay-safe fields and indexes: source raw evidence references, parser versions, deterministic fingerprints, an `evidence_refs` column for `iac_scan_findings`, and duplicate-prevention indexes for parser rows and evidence links.
...
Phase 2B fixture ingestion and Phase 2E verified webhook persistence write new deployment runs and raw evidence metadata at runtime; the seed file remains deterministic static demo data. Phase 3C does not change seed data.
