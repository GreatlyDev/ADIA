...
These phases do not wire Supabase into the browser application. They also do not add parser execution, deterministic analyzer execution, LLM generation, live dashboard queries, Edge Functions, artifact download, or Terraform apply behavior.
...
Phase 2B fixture ingestion and Phase 2E verified webhook persistence write new deployment runs and raw evidence metadata at runtime; the seed file remains deterministic static demo data.
